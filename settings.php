...
if ($hassiteconfig) {
    ...
    $settings->add(new admin_setting_configtext('logstore_xapi/maxbatchsize',
      ...
      $settings->add(new admin_setting_configcheckbox('logstore_xapi/whitelistenabled',
                        'Whitelisting Students Enabled',
                        'Only allowed whitelisted moodle student ids to be sent', 0));

      $settings->add(new admin_setting_configtext('logstore_xapi/whitelist',
                                          'Whitelist of student ids (comma seperated)', '', '', PARAM_TEXT));

      $settings->add(new admin_setting_configcheckbox('logstore_xapi/whitelistcoursesenabled',
                          'Whitelisting Courses Enabled',
                          'Only allowed whitelisted moodle course ids to be sent', 0));

      $settings->add(new admin_setting_configtext('logstore_xapi/whitelistcourses',
